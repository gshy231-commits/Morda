...
import { Eye, EyeOff, Mail, Lock, User, ArrowRight, Check, CheckCircle, ArrowLeft } from "lucide-react";
...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen bg-[#030306] flex">
      ...
      {}
      <div className="hidden lg:flex flex-1 items-center justify-center bg-gradient-to-br from-[#4488ff]/5 to-[#00ffaa]/5 relative overflow-hidden">
        <div className="absolute inset-0 bg-[url('/grid.svg')] opacity-10" />
        <div className="absolute top-1/3 left-1/4 w-96 h-96 bg-[#4488ff]/20 rounded-full blur-[150px]" />
        <div className="absolute bottom-1/3 right-1/4 w-96 h-96 bg-[#00ffaa]/20 rounded-full blur-[150px]" />
        ...
        <motion.div 
          ...
          initial="hidden"
          animate="visible"
          className="absolute z-20 p-12 max-w-md pointer-events-none"
        >
          ...
          <ul className="space-y-4">
            {features.map((feature, i) => (
              <motion.li 
                ...
                className="flex items-center gap-3 text-[#8a8a9f]"
              >
                <div className="w-6 h-6 rounded-full bg-[#00ffaa]/20 flex items-center justify-center">
      ...
      <div className="flex-1 flex items-center justify-center p-8 relative z-10">
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <motion.div variants={itemVariants}>
              <label className="block text-sm text-[#8a8a9f] mb-2">Full Name</label>
              <div className="relative">
                <User className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-[#5a5a6f]" />
                <input
                  type="text"
                  value={name}
              ...
              <motion.div variants={itemVariants} className="mt-3 space-y-1">
                {passwordRequirements.map((req, i) => (
                  <div key={i} className="flex items-center gap-2 text-xs">
                    <motion.div 
                      animate={{ 
                        backgroundColor: req.met ? "rgba(0, 255, 170, 0.2)" : "rgba(255, 255, 255, 0.05)",
                        scale: req.met ? [1, 1.2, 1] : 1
                      }}
                      transition={{ duration: 0.2 }}
                      className="w-4 h-4 rounded-full flex items-center justify-center"
                    >
                      {req.met && <Check className="w-3 h-3 text-[#00ffaa]" />}
                    </motion.div>
                    <span className={req.met ? 'text-[#00ffaa]' : 'text-[#5a5a6f]'}>{req.text}</span>
                  </div>
                ))}
              </motion.div>
            </motion.div>
          ...
          <motion.p variants={itemVariants} className="text-center text-[#5a5a6f] text-xs mt-6">
            By creating an account, you agree to our{" "}
            <Link href="/terms" className="text-[#00ffaa] hover:underline">Terms</Link>
            {" "}and{" "}
            <Link href="/privacy" className="text-[#00ffaa] hover:underline">Privacy Policy</Link>
          </motion.p>

          <motion.p variants={itemVariants} className="text-center text-[#6a6a7f] mt-6">
